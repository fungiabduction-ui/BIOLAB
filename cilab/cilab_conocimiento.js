...
function _creMetabolicFasesHTML(formulaId, geneticaId) {
  ...
  var _fCtxM  = _sp.frasco;
  var ciTotal  = _creGetPlacasFromCI(formulaId, geneticaId, _fCtxM ? _fCtxM.expId : null, _fCtxM ? _fCtxM.frascoLabel : null);
  var inocDate = _creInoculacionDate(formulaId, geneticaId);

  // Aggregate incidence across all selected cepas (multi-select mode)
  var aggInc = {}; // faseId -> { obs, total }
  if (_sp.selected.size > 1) {
    _sp.selected.forEach(function(key) {
      var kp = key.split('|');
      var gId = kp[2];
      _creFasesRead(formulaId, gId).forEach(function(f) {
        if (!aggInc[f.fase]) aggInc[f.fase] = { obs: 0, total: 0, hasObs: false, hasTotal: false };
        if (f.placasObservadas != null) { aggInc[f.fase].obs += f.placasObservadas; aggInc[f.fase].hasObs = true; }
        if (f.totalPlacas      != null) { aggInc[f.fase].total += f.totalPlacas;    aggInc[f.fase].hasTotal = true; }
      });
    });
  }
  ...
  var nextIdx = -1;
  for (var i = 0; i < _FASES_DEF.length; i++) {
    if (!regMap[_FASES_DEF[i].id]) { nextIdx = i; break; }
  }

  var fIdE = esc(formulaId);
  var gIdE = esc(geneticaId);
  var html = '<div class="cre-fas-wrap">';

  _FASES_DEF.forEach(function(def, idx) {
    var reg     = regMap[def.id];
    var isDone  = !!reg;
    var isNext  = idx === nextIdx;
    var isFuture = !isDone && !isNext;
    var isLast  = idx === _FASES_DEF.length - 1;

    html += '<div class="cre-fas-row' + (isFuture ? ' cre-fas-row--future' : '') + '">';

    html += '<div class="cre-fas-left">';
    var dotStyle = isDone
      ? (reg.auto === 'inferred'
          ? 'background:#444;border:1.5px dashed #666'
          : 'background:' + def.color + ';box-shadow:0 0 6px ' + def.color + '55')
      : '';
    html += '<div class="cre-fas-dot'
      + (isDone ? ' cre-fas-dot--done' : isNext ? ' cre-fas-dot--next' : ' cre-fas-dot--future') + '"'
      + (dotStyle ? ' style="' + dotStyle + '"' : '') + '></div>';
    if (!isLast) html += '<div class="cre-fas-line" style="' + (isDone ? 'background:' + def.color + '55' : '') + '"></div>';
    html += '</div>';

    html += '<div class="cre-fas-content">';

    if (isDone) {
      var isInferred = reg.auto === 'inferred';
      var dd = reg.fecha ? reg.fecha.split('-') : null;
      var fechaFmt = dd ? (dd[2] + '/' + dd[1]) : (isInferred ? 'Desconocido' : '');
      // Recompute dia from fecha vs inoculación
      var displayDia = isInferred ? '~' + reg.dia : reg.dia;
      var isBadDate  = false;
      if (reg.fecha && inocDate && def.id !== 'inoculacion') {
        var d0r = new Date(inocDate); d0r.setHours(0,0,0,0);
        var d1r = new Date(reg.fecha); d1r.setHours(0,0,0,0);
        var rawDia = Math.floor((d1r - d0r) / 86400000);
        if (rawDia < 0) { isBadDate = true; displayDia = '!'; }
        else { displayDia = rawDia; }
      }
      var dotColor = isBadDate ? '#ff4444' : isInferred ? '#555' : def.color;
      html += '<div class="cre-fas-header">'
        + '<span class="cre-fas-name" style="color:' + dotColor + ';' + (isInferred ? 'opacity:0.6' : '') + '">' + esc(def.label) + '</span>'
        + '<div class="cre-fas-meta-row">'
        + (isBadDate
            ? '<span class="cre-fas-day" style="color:#ff4444" title="Fecha anterior a inoculación — corregir">⚠ anterior a inoc.</span>'
            : '<span class="cre-fas-day" style="color:' + dotColor + '">Día ' + displayDia + '</span>')
        + '<span class="cre-fas-date" style="' + (isInferred ? 'opacity:0.5' : '') + (isBadDate ? 'color:#ff4444' : '') + '">' + fechaFmt + '</span>'
        + '<button class="cre-fas-calib-btn" onclick="creEditFase(\'' + fIdE + '\',\'' + gIdE + '\',\'' + esc(def.id) + '\')" title="' + (isBadDate ? 'Corregir fecha inválida' : isInferred ? 'Registrar fecha real' : 'Calibrar fecha') + '">✎</button>'
        + '</div></div>';
      if (def.sub) html += '<div class="cre-fas-sub"' + (def.id === 'colonizacion_completa' ? ' style="color:#00CC99;font-weight:600"' : '') + '>' + esc(def.sub) + '</div>';
      // Aggregate incidence when multi-select, per-cepa otherwise
      var showObs = reg.placasObservadas, showTotal = reg.totalPlacas;
      if (_sp.selected.size > 1 && aggInc[def.id]) {
        var ai = aggInc[def.id];
        if (ai.hasObs)   showObs   = ai.obs;
        if (ai.hasTotal) showTotal = ai.total;
      }
      if (showObs != null && showTotal != null && showTotal > 0) {
        var pct = Math.round((showObs / showTotal) * 100);
        html += '<div class="cre-fas-incidence">'
          + '<span class="cre-fas-inc-label">' + showObs + '/' + showTotal + '</span>'
          + '<div class="cre-fas-inc-track"><div class="cre-fas-inc-fill" style="width:' + pct + '%;background:' + def.color + '"></div></div>'
          + '<span class="cre-fas-inc-pct" style="color:' + def.color + '">' + pct + '%</span>'
          + '</div>';
      }
      html += '<div id="cre-fase-editform-' + esc(def.id) + '" style="display:none"></div>';

    } else if (isNext) {
      var diaLbl = diasAct != null ? 'Día ' + diasAct : 'Hoy';
      html += '<div class="cre-fas-header">'
        + '<span class="cre-fas-name cre-fas-name--next">' + esc(def.label) + '</span>';
      if (def.autoCI && def.id !== 'inoculacion') {
        html += '<span class="cre-fas-auto-badge">AUTO · CI</span>';
      } else {
        var btnLabel = (def.id === 'inoculacion') ? '+ Registrar fecha' : ('+ ' + diaLbl);
        html += '<button class="cre-fas-reg-btn" id="cre-fase-regbtn-' + esc(def.id) + '"'
          + ' onclick="creRegisterFase(\'' + fIdE + '\',\'' + gIdE + '\',\'' + esc(def.id) + '\')">'
          + btnLabel + '</button>';
      }
      html += '</div>';
      if (def.sub) html += '<div class="cre-fas-sub">' + esc(def.sub) + '</div>';
      if (def.id === 'inoculacion') {
        html += '<div style="font-size:9px;color:var(--tx3);margin-top:2px">No se detectó fecha en CI — ingresala manualmente.</div>';
      }
      html += '<div id="cre-fase-incform-' + esc(def.id) + '" style="display:none"></div>';

    } else {
      html += '<div class="cre-fas-header">'
        + '<span class="cre-fas-name cre-fas-name--future">' + esc(def.label) + '</span>'
        + (def.autoCI ? '<span class="cre-fas-auto-badge" style="opacity:.45">AUTO · CI</span>' : '')
        + '</div>';
      if (def.sub) html += '<div class="cre-fas-sub cre-fas-sub--future">' + esc(def.sub) + '</div>';
    }
  ...
  if (fases.length >= 2) {
    var activ = regMap['actividad_metabolica'];
    var filam = regMap['primeros_filamentos'];
    var rizo  = regMap['rizomorfismo_evaluable'];
    html += '<div class="cre-motor-strip"><div class="cre-motor-label">Motor</div><div class="cre-motor-grid">';
    if (activ) {
      html += '<div class="cre-motor-card">'
        + '<div class="cre-motor-title">Inicio metabólico</div>'
        + '<div class="cre-motor-val" style="color:#00cc33">Día ' + activ.dia + '</div>'
        + '</div>';
    }
    if (filam && activ) {
      html += '<div class="cre-motor-card">'
        + '<div class="cre-motor-title">Vel. organización</div>'
        + '<div class="cre-motor-val" style="color:var(--ac4)">' + (filam.dia - activ.dia) + 'd</div>'
        + '<div class="cre-motor-sub">Metab→Filam.</div></div>';
    }
    if (rizo) {
      html += '<div class="cre-motor-card">'
        + '<div class="cre-motor-title">Días hasta rizo</div>'
        + '<div class="cre-motor-val" style="color:var(--ac2)">Día ' + rizo.dia + '</div></div>';
    } else if (diasAct != null) {
      html += '<div class="cre-motor-card">'
        + '<div class="cre-motor-title">Proyección rizo</div>'
        + '<div class="cre-motor-val" style="color:var(--ac2)">~Día ' + (diasAct + 4) + '</div>'
        + '<div class="cre-motor-sub">estimado</div></div>';
    }
    html += '</div></div>';
  }
...
// ── Fases Scrubber (hybrid compact+interactive) ──────────────────────────────
window._creScrubbingState = null;

function _creFasesScrubberHTML(formulaId, geneticaId) {
  var maxDay = 35;
  var fIdE   = esc(formulaId);
  var gIdE   = esc(geneticaId);
  var fases  = _creFasesRead(formulaId, geneticaId);
  ...
  var html = '<div class="cre-fase-scrubber">';
  html += '<div class="cre-scrubber-label">Fases · arrastrá para calibrar</div>';
  html += '<div class="cre-scrub-wrap">';
  html += '<div class="cre-scrub-track" id="cre-scrub-track-' + fIdE + '-' + gIdE + '">';
  html += '<div class="cre-scrub-rail"></div>';

  [0, 7, 14, 21, 28, 35].forEach(function(d) {
    html += '<div class="cre-scrub-tick" style="left:' + (d / maxDay * 100) + '%"><span>' + (d === 0 ? 'Inoc' : 'D' + d) + '</span></div>';
  });

  _FASES_DEF.forEach(function(def) {
    var reg    = regMap[def.id];
    var dia    = reg ? (reg.dia != null ? reg.dia : def.typicalDay) : def.typicalDay;
    dia = Math.max(0, Math.min(maxDay, dia));
    var pct    = (dia / maxDay) * 100;
    var isDone = !!reg && reg.auto !== 'inferred';
    var isInf  = reg && reg.auto === 'inferred';
    var dotBg  = isDone ? def.color : 'transparent';
    var dotBrd = isDone ? '2px solid ' + def.color : (isInf ? '2px dashed #555' : '2px dashed #333');
    var dotGlw = isDone ? '0 0 5px ' + def.color + '55' : 'none';
    var faseIdE = esc(def.id);
    var isDrag  = def.id !== 'inoculacion';

    html += '<div class="cre-scrub-dot" data-fase-id="' + faseIdE + '" style="left:' + pct + '%"'
      + (isDrag ? ' onpointerdown="creScrubStart(event,\'' + fIdE + '\',\'' + gIdE + '\',\'' + faseIdE + '\')"' : '')
      + (isDone && isDrag ? ' ondblclick="creDeleteFaseFromScrubber(event,\'' + fIdE + '\',\'' + gIdE + '\',\'' + faseIdE + '\')"' : '')
      + ' title="' + esc(def.label) + (isDone && isDrag ? ' — Día ' + dia + ' · doble click para eliminar' : (isDone ? ' — Día ' + dia : (isInf ? ' ~D' + dia : ' D típ.' + def.typicalDay))) + '">'
      + '<div class="cre-scrub-dot-inner" style="background:' + dotBg + ';border:' + dotBrd + ';box-shadow:' + dotGlw + ';cursor:' + (isDrag ? 'grab' : 'default') + '"></div>'
      + '<span class="cre-scrub-dot-name" style="color:' + (isDone ? def.color : '#555') + '">' + esc(def.label.split(' ')[0]) + '</span>'
      + '<span class="cre-scrub-day-lbl" style="color:' + (isDone ? def.color : '#444') + '">D' + dia + '</span>'
      + '</div>';
  });

  // Dot "Hoy" — guía visual del día transcurrido desde inoculación, sin registro
  var diasHoy = _creDiasSinceInoc(formulaId, geneticaId);
  if (diasHoy !== null) {
    var hoyPct = Math.min(100, (diasHoy / maxDay) * 100);
    html += '<div class="cre-scrub-dot cre-scrub-hoy" style="left:' + hoyPct + '%" title="Hoy — D' + diasHoy + '">'
      + '<div class="cre-scrub-dot-inner" style="background:#FFD700;border:2px solid #FFD700;cursor:default"></div>'
      + '<span class="cre-scrub-dot-name" style="color:#FFD700;font-weight:700">Hoy</span>'
      + '<span class="cre-scrub-day-lbl" style="color:#FFD700">D' + diasHoy + '</span>'
      + '</div>';
  }
...
function creScrubStart(evt, formulaId, geneticaId, faseId) {
  if (faseId === 'inoculacion') return;
  evt.preventDefault();
  evt.stopPropagation();
  var trackEl = document.getElementById('cre-scrub-track-' + esc(formulaId) + '-' + esc(geneticaId));
  if (!trackEl) return;
  window._creScrubbingState = { formulaId: formulaId, geneticaId: geneticaId, faseId: faseId, trackEl: trackEl, maxDay: 35 };
  document.addEventListener('pointermove', _creOnScrubMove);
...
function _creOnScrubMove(evt) {
  ...
  if (!s) return;
  var rect = s.trackEl.getBoundingClientRect();
  var pct  = Math.max(0, Math.min(1, (evt.clientX - rect.left) / rect.width));
  var dia  = Math.round(pct * s.maxDay);
  var dotEl = s.trackEl.querySelector('[data-fase-id="' + s.faseId + '"]');
  if (dotEl) {
    dotEl.style.left = (pct * 100) + '%';
    var lbl = dotEl.querySelector('.cre-scrub-day-lbl');
    if (lbl) lbl.textContent = 'D' + dia;
  }
}

function _creOnScrubEnd(evt) {
  var s = window._creScrubbingState;
  if (!s) return;
  document.removeEventListener('pointermove', _creOnScrubMove);
  document.removeEventListener('pointerup', _creOnScrubEnd);
  ...
  window._creScrubbingState = null;
  if (s.isBatch) {
    // Solo stagea — el usuario debe confirmar antes de guardar en todas las cepas
    _sp.batchStagedFase = { faseId: s.faseId, dia: dia };
    var bw = document.getElementById('cre-batch-controls-' + esc(s.formulaId));
    if (bw) bw.innerHTML = _creBatchControlsHTML(s.formulaId);
  } else {
    creFaseScrubSave(s.formulaId, s.geneticaId, s.faseId, dia);
  }
}

function creFaseScrubSave(formulaId, geneticaId, faseId, dia) {
  ...
  var fecha    = today;
  if (inocDate) {
    var d0 = new Date(inocDate + 'T12:00:00');
    d0.setDate(d0.getDate() + dia);
    var comp = d0.toISOString().slice(0, 10);
    if (comp <= today) fecha = comp;
  }
  var order    = _FASES_DEF.map(function(f) { return f.id; });
  var existing = fases.find(function(f) { return f.fase === faseId; });
  if (existing) {
    existing.dia   = dia;
    existing.fecha = fecha;
    existing.ts    = now();
    if (existing.auto === 'inferred') delete existing.auto;
  } else {
    fases.push({ fase: faseId, dia: dia, fecha: fecha, ts: now() });
    fases.sort(function(a, b) { return order.indexOf(a.fase) - order.indexOf(b.fase); });
  }
  _creFasesWrite(formulaId, geneticaId, fases);
  _creLogFase(formulaId, geneticaId, faseId, dia, fecha, !!existing, _sp.frasco);
  notif('Fase guardada · D' + dia, 'info');
  _creRenderCepasSection(formulaId);
}

function creDeleteFaseFromScrubber(evt, formulaId, geneticaId, faseId) {
  evt.preventDefault();
...
function creBatchScrubStart(evt, formulaId, faseId) {
  if (faseId === 'inoculacion') return;
  evt.preventDefault();
  evt.stopPropagation();
  var trackEl = document.getElementById('cre-batch-scrub-track-' + esc(formulaId));
  if (!trackEl) return;
  window._creScrubbingState = { formulaId: formulaId, geneticaId: null, faseId: faseId, trackEl: trackEl, maxDay: 35, isBatch: true };
  document.addEventListener('pointermove', _creOnScrubMove);
  document.addEventListener('pointerup', _creOnScrubEnd);
}
...
function _creBatchFaseScrubSave(formulaId, faseId, dia) {
  var order    = _FASES_DEF.map(function(f) { return f.id; });
  var today    = new Date().toISOString().slice(0, 10);
  _sp.selected.forEach(function(key) {
    var gId    = key.split('|')[2];
    ...
    var existing = fases.find(function(f) { return f.fase === faseId; });
    if (existing) {
      existing.dia = dia; existing.fecha = fecha; existing.ts = now();
      if (existing.auto === 'inferred') delete existing.auto;
    } else {
      fases.push({ fase: faseId, dia: dia, fecha: fecha, ts: now() });
      fases.sort(function(a, b) { return order.indexOf(a.fase) - order.indexOf(b.fase); });
    ...
    _creLogFase(formulaId, gId, faseId, dia, fecha, !!existing, _sp.frasco);
  });
  _sp.batchFasePos[faseId] = dia;
  notif('Fase registrada en ' + _sp.selected.size + ' cepas · D' + dia, 'info');
  _creRenderCepasSection(formulaId);
  if (_sp.formulaId) _creRenderLogSection(_sp.formulaId);
}

function _creBatchFasesScrubberHTML(formulaId) {
  var maxDay    = 35;
  var fIdE      = esc(formulaId);
  var savedPos  = _sp.batchFasePos || {};
  var html = '<div class="cre-fase-scrubber" style="margin-top:10px">';
  html += '<div class="cre-scrubber-label">Registrar fase en batch · arrastrá el punto</div>';
  html += '<div class="cre-scrub-wrap">';
  html += '<div class="cre-scrub-track" id="cre-batch-scrub-track-' + fIdE + '">';
  html += '<div class="cre-scrub-rail"></div>';
  [0, 7, 14, 21, 28, 35].forEach(function(d) {
    html += '<div class="cre-scrub-tick" style="left:' + (d / maxDay * 100) + '%"><span>' + (d === 0 ? 'Inoc' : 'D' + d) + '</span></div>';
  });
  var staged   = _sp.batchStagedFase;
  _FASES_DEF.forEach(function(def) {
    var isStaged = staged && staged.faseId === def.id;
    var hasSaved = !isStaged && savedPos[def.id] != null;
    var dia    = isStaged ? staged.dia : (hasSaved ? savedPos[def.id] : def.typicalDay);
    dia = Math.max(0, Math.min(maxDay, dia));
    var pct    = (dia / maxDay) * 100;
    var isDrag = def.id !== 'inoculacion';
    var faseIdE = esc(def.id);
    var dotBg  = hasSaved ? def.color : 'transparent';
    var dotBrd = isStaged ? '2px solid #FFD700' : (hasSaved ? '2px solid ' + def.color : '2px dashed ' + def.color);
    var dotGlw = isStaged ? '0 0 6px #FFD70099' : (hasSaved ? '0 0 5px ' + def.color + '55' : 'none');
    var dotAnim = isStaged ? 'animation:cre-hoy-pulse 1s ease-in-out infinite;' : '';
    html += '<div class="cre-scrub-dot" data-fase-id="' + faseIdE + '" style="left:' + pct + '%"'
      + (isDrag && !staged ? ' onpointerdown="creBatchScrubStart(event,\'' + fIdE + '\',\'' + faseIdE + '\')"' : '')
      + ' title="' + esc(def.label) + (isStaged ? ' — D' + dia + ' · pendiente confirmar' : (hasSaved ? ' — D' + dia + ' (guardado)' : ' · D típ.' + dia)) + '">'
      + '<div class="cre-scrub-dot-inner" style="background:' + dotBg + ';border:' + dotBrd + ';box-shadow:' + dotGlw + ';' + dotAnim + 'cursor:' + (isDrag && !staged ? 'grab' : 'default') + '"></div>'
      + '<span class="cre-scrub-dot-name" style="color:' + (isStaged ? '#FFD700' : def.color) + '">' + esc(def.label.split(' ')[0]) + '</span>'
      + '<span class="cre-scrub-day-lbl" style="color:' + def.color + '">D' + dia + '</span>'
      + '</div>';
  });

  // Dot "Hoy" usando la fecha de inoculación de la primera cepa seleccionada
  var _bFirstKey = _sp.selected.size > 0 ? Array.from(_sp.selected)[0] : null;
  var _bFirstGId = _bFirstKey ? _bFirstKey.split('|')[2] : null;
  var _diasHoyB  = _bFirstGId ? _creDiasSinceInoc(formulaId, _bFirstGId) : null;
  if (_diasHoyB !== null) {
    var hoyBPct = Math.min(100, (_diasHoyB / maxDay) * 100);
    html += '<div class="cre-scrub-dot cre-scrub-hoy" style="left:' + hoyBPct + '%" title="Hoy — D' + _diasHoyB + '">'
      + '<div class="cre-scrub-dot-inner" style="background:#FFD700;border:2px solid #FFD700;cursor:default"></div>'
      + '<span class="cre-scrub-dot-name" style="color:#FFD700;font-weight:700">Hoy</span>'
      + '<span class="cre-scrub-day-lbl" style="color:#FFD700">D' + _diasHoyB + '</span>'
      + '</div>';
  }
...
function creRegisterFase(formulaId, geneticaId, faseId) {
  var _fCtxR = _sp.frasco;
  var ciTotal = _creGetPlacasFromCI(formulaId, geneticaId, _fCtxR ? _fCtxR.expId : null, _fCtxR ? _fCtxR.frascoLabel : null);
  // In multi-select mode show aggregate across all selected cepas
  if (_sp.selected.size > 1) {
    var ciAgg = 0; var ciAggHas = false;
    _sp.selected.forEach(function(key) {
      var kp = key.split('|');
      var ci = _creGetPlacasFromCI(formulaId, kp[2], kp[0] || null, kp[1] || null);
      if (ci != null) { ciAgg += ci; ciAggHas = true; }
    });
    if (ciAggHas) ciTotal = ciAgg;
  }
  var diasAct = _creDiasSinceInoc(formulaId, geneticaId) || 0;
  var fIdE    = esc(formulaId);
  var gIdE    = esc(geneticaId);
  var faseIdE = esc(faseId);

  var btn     = document.getElementById('cre-fase-regbtn-' + faseId);
  var incForm = document.getElementById('cre-fase-incform-' + faseId);
  if (!incForm) return;
  if (btn) btn.style.display = 'none';
  incForm.style.display = '';

  var todayIso = new Date().toISOString().slice(0, 10);
  // Estimate date from inoculación + typicalDay for retroactive entry
  var faseDef  = _FASES_DEF.find(function(d) { return d.id === faseId; });
  var defaultDate = todayIso;
  if (faseId !== 'inoculacion') {
    var inocDateR = _creInoculacionDate(formulaId, geneticaId);
    if (inocDateR && faseDef && faseDef.typicalDay != null) {
      var estD = new Date(inocDateR);
      estD.setDate(estD.getDate() + faseDef.typicalDay);
      var estStr = estD.toISOString().slice(0, 10);
      if (estStr <= todayIso) defaultDate = estStr;
    }
  }

  var confirmCall = 'creRegisterFaseConfirm(\'' + fIdE + '\',\'' + gIdE + '\',\'' + faseIdE + '\')';
  var cancelCall  = 'creRegisterFaseCancel(\'' + faseIdE + '\')';

  if (faseId === 'inoculacion') {
    incForm.innerHTML = '<div class="cre-fas-inc-input-row">'
      + '<span class="cre-fas-inc-lbl">Fecha inoculación:</span>'
      + '<input type="date" id="cre-fase-fecha-' + faseIdE + '" class="cre-incidence-input" style="width:130px;font-size:11px" value="' + defaultDate + '">'
      + '<button class="clab-btn clab-btn-sm" style="background:var(--ac2);color:var(--bg);border-color:var(--ac2)" onclick="' + confirmCall + '">✓</button>'
      + '<button class="clab-btn clab-btn-sm" onclick="' + cancelCall + '">✕</button>'
      + '</div>';

  } else if (faseId === 'colonizacion_completa') {
    var estDtLocal = new Date(new Date(defaultDate + 'T12:00:00').getTime()
      - new Date().getTimezoneOffset() * 60000).toISOString().slice(0, 16);
    incForm.innerHTML = '<div style="display:flex;flex-direction:column;gap:6px;margin-top:4px">'
      + '<div style="font-size:9px;color:var(--tx3)">Registrá la fecha y hora exacta de observación</div>'
      + '<div class="cre-fas-inc-input-row">'
      + '<span class="cre-fas-inc-lbl">Fecha y hora:</span>'
      + '<input type="datetime-local" id="cre-fase-fecha-' + faseIdE + '" class="cre-incidence-input" style="width:170px;font-size:11px" value="' + estDtLocal + '">'
      + '</div>'
      + '<div class="cre-fas-inc-input-row">'
      + '<button class="clab-btn clab-btn-sm" style="background:var(--ac2);color:var(--bg);border-color:var(--ac2)" onclick="' + confirmCall + '">✓ Confirmar</button>'
      + '<button class="clab-btn clab-btn-sm" onclick="' + cancelCall + '">✕</button>'
      + '</div>'
      + '</div>';

  } else {
    incForm.innerHTML = '<div style="display:flex;flex-direction:column;gap:5px;margin-top:4px">'
      + '<div class="cre-fas-inc-input-row">'
      + '<span class="cre-fas-inc-lbl">Fecha:</span>'
      + '<input type="date" id="cre-fase-fecha-' + faseIdE + '" class="cre-incidence-input" style="width:120px;font-size:11px" value="' + defaultDate + '">'
      + '</div>'
      + '<div class="cre-fas-inc-input-row">'
      + '<span class="cre-fas-inc-lbl">Placas:</span>'
      + '<input type="number" id="cre-fase-plobs-' + faseIdE + '" class="cre-incidence-input" style="width:52px;font-size:11px" min="0" max="999"'
      + (ciTotal != null ? ' value="' + ciTotal + '"' : ' placeholder="0"') + '>'
      + (ciTotal != null ? '<span style="font-size:9px;color:var(--tx3)">/ ' + ciTotal + '</span>' : '')
      + '<button class="clab-btn clab-btn-sm" style="background:var(--ac2);color:var(--bg);border-color:var(--ac2)" onclick="' + confirmCall + '">✓</button>'
      + '<button class="clab-btn clab-btn-sm" onclick="' + cancelCall + '">✕</button>'
      + '</div>'
      + '</div>';
...
function creRegisterFaseConfirm(formulaId, geneticaId, faseId) {
  _creLiftCleared(formulaId); // usuario empieza de nuevo — levantar tombstone
  var _fCtxC = _sp.frasco;
  var ciTotal  = _creGetPlacasFromCI(formulaId, geneticaId, _fCtxC ? _fCtxC.expId : null, _fCtxC ? _fCtxC.frascoLabel : null);
  var fechaEl  = document.getElementById('cre-fase-fecha-' + faseId);
  var obsEl    = document.getElementById('cre-fase-plobs-' + faseId);
  var today    = new Date().toISOString().slice(0, 10);
  var fechaStr = fechaEl ? fechaEl.value.slice(0, 10) : today;
  if (!fechaStr) fechaStr = today;

  // Compute aggregate CI for proportional per-cepa scaling in multi-select mode
  var isMulti = _sp.selected.size > 1;
  var ciTotalAgg = 0;
  if (isMulti) {
    _sp.selected.forEach(function(key) {
      var kp = key.split('|');
      var ci = _creGetPlacasFromCI(formulaId, kp[2], kp[0] || null, kp[1] || null);
      if (ci != null) ciTotalAgg += ci;
    });
  }

  var entry;
  if (faseId === 'inoculacion') {
    entry = { fase: 'inoculacion', dia: 0, fecha: fechaStr, ts: now(), auto: false };
  } else {
    var dia = 0;
    var inocDate = _creInoculacionDate(formulaId, geneticaId);
    if (inocDate) {
    ...
    var placasObsAgg = (obsEl && obsEl.value !== '') ? parseInt(obsEl.value, 10) : null;
    // For single cepa, placasObs = user input directly. For multi, scaled per-cepa below.
    entry = {
      fase: faseId,
      dia:  dia,
      fecha: fechaStr,
      placasObservadas: (!isNaN(placasObsAgg) && placasObsAgg != null) ? placasObsAgg : null,
      totalPlacas: ciTotal != null ? ciTotal : ((!isNaN(placasObsAgg) && placasObsAgg != null) ? placasObsAgg : null),
      ts: now(),
    };
  }
  ...
  // Build list of cepas to register: all selected (if multi), or just the active one
  var targets = [];
  if (isMulti) {
    _sp.selected.forEach(function(key) { targets.push(key.split('|')[2]); });
  } else {
    targets.push(geneticaId);
  }

  var placasObsAgg2 = entry.placasObservadas; // aggregate value from input

  var saved = 0;
  targets.forEach(function(gId) {
    if (!gId) return;
    var tEntry = entry;
    // Recalculate dia per-cepa if dates can differ
    if (faseId !== 'inoculacion') {
      var inocDateT = _creInoculacionDate(formulaId, gId);
      if (inocDateT && fechaStr) {
        var d0t = new Date(inocDateT); d0t.setHours(0,0,0,0);
        var d1t = new Date(fechaStr);  d1t.setHours(0,0,0,0);
        var diaT = Math.max(0, Math.floor((d1t - d0t) / 86400000));
        var ciT  = _creGetPlacasFromCI(formulaId, gId, _fCtxC ? _fCtxC.expId : null, _fCtxC ? _fCtxC.frascoLabel : null);
        // Scale placasObservadas proportionally per cepa so percentages are correct
        var pObsT = placasObsAgg2;
        if (isMulti && placasObsAgg2 != null && ciTotalAgg > 0 && ciT != null) {
          pObsT = Math.round(placasObsAgg2 * ciT / ciTotalAgg);
        }
        tEntry = Object.assign({}, entry, { dia: diaT, placasObservadas: pObsT });
        if (faseId !== 'colonizacion_completa' && faseId !== 'inoculacion') {
          tEntry.totalPlacas = ciT != null ? ciT : entry.totalPlacas;
        }
      }
    }
    var fasesG = _creFasesRead(formulaId, gId).filter(function(f) { return f.fase !== faseId; });
    fasesG.push(tEntry);
    fasesG.sort(function(a, b) { return order.indexOf(a.fase) - order.indexOf(b.fase); });
    _creFasesWrite(formulaId, gId, fasesG);
    _creLogFase(formulaId, gId, faseId, tEntry.dia, tEntry.fecha, false, _sp.frasco);
    if (faseId === 'colonizacion_completa') {
      _creSyncColonizacionToCI(formulaId, gId, fechaStr);
    }
    saved++;
  });

  var msg = saved > 1
    ? 'Fase registrada en ' + saved + ' cepas · Día ' + entry.dia
    : 'Fase registrada · Día ' + entry.dia;
  notif(msg, 'info');
...
function creRegisterFaseCancel(faseId) {
  var btn     = document.getElementById('cre-fase-regbtn-' + faseId);
  var incForm = document.getElementById('cre-fase-incform-' + faseId);
  if (btn) btn.style.display = '';
  if (incForm) { incForm.style.display = 'none'; incForm.innerHTML = ''; }
}

// ── Calibración retroactiva de fase ya registrada ────────────────────────────

function creEditFase(formulaId, geneticaId, faseId) {
  var fIdE    = esc(formulaId);
  var gIdE    = esc(geneticaId);
  var faseIdE = esc(faseId);
  var fases   = _creFasesRead(formulaId, geneticaId);
  ...
  if (!reg) return;
  var editForm = document.getElementById('cre-fase-editform-' + faseId);
  if (!editForm) return;
  if (editForm.style.display !== 'none') { editForm.style.display = 'none'; editForm.innerHTML = ''; return; }
  var currentFecha = reg.fecha || new Date().toISOString().slice(0, 10);
  editForm.style.display = '';
  editForm.innerHTML = '<div class="cre-fase-editbox">'
    + '<div style="font-size:9px;color:var(--tx3);margin-bottom:5px">Calibrar fecha real de observación (recalcula días)</div>'
    + '<div class="cre-fas-inc-input-row">'
    + '<span class="cre-fas-inc-lbl">Fecha:</span>'
    + '<input type="date" id="cre-fase-editfecha-' + faseIdE + '" class="cre-incidence-input" style="width:120px;font-size:11px" value="' + currentFecha + '">'
    + (reg.placasObservadas != null && faseId !== 'inoculacion'
        ? '<span class="cre-fas-inc-lbl" style="margin-left:8px">Placas:</span>'
          + '<input type="number" id="cre-fase-editplobs-' + faseIdE + '" class="cre-incidence-input" style="width:50px;font-size:11px" value="' + reg.placasObservadas + '" min="0" max="999">'
        : '')
    + '<button class="clab-btn clab-btn-sm" style="background:var(--ac2);color:var(--bg);border-color:var(--ac2)" onclick="creEditFaseConfirm(\'' + fIdE + '\',\'' + gIdE + '\',\'' + faseIdE + '\')">✓</button>'
    + '<button class="clab-btn clab-btn-sm" onclick="creEditFaseCancel(\'' + faseIdE + '\')">✕</button>'
    + '</div></div>';
}

function creEditFaseConfirm(formulaId, geneticaId, faseId) {
  var fechaEl  = document.getElementById('cre-fase-editfecha-' + faseId);
  var plobsEl  = document.getElementById('cre-fase-editplobs-' + faseId);
  if (!fechaEl || !fechaEl.value) return;
  var nuevaFecha = fechaEl.value;
  var fases = _creFasesRead(formulaId, geneticaId);

  if (faseId === 'inoculacion') {
    var d0new = new Date(nuevaFecha); d0new.setHours(0,0,0,0);
    fases.forEach(function(f) {
      if (f.fase === 'inoculacion') {
        f.fecha = nuevaFecha; f.dia = 0; f.inoculationDate = nuevaFecha;
      } else if (f.fecha) {
    ...
    if (inocDate) {
      var d0 = new Date(inocDate); d0.setHours(0,0,0,0);
      var d1 = new Date(nuevaFecha); d1.setHours(0,0,0,0);
      dia = Math.max(0, Math.floor((d1 - d0) / 86400000));
    }
    fases.forEach(function(f) {
      if (f.fase === faseId) {
        f.fecha = nuevaFecha; f.dia = dia;
        if (plobsEl && plobsEl.value !== '') f.placasObservadas = parseInt(plobsEl.value, 10);
      }
    });
  }

  _creFasesWrite(formulaId, geneticaId, fases);
  var _editedFase = fases.find(function(f) { return f.fase === faseId; });
  var _editedDia  = faseId === 'inoculacion' ? 0 : (_editedFase ? _editedFase.dia : 0);
  _creLogFase(formulaId, geneticaId, faseId, _editedDia, _editedFase ? _editedFase.fecha : null, true, _sp.frasco);
  notif('Fecha calibrada · Día ' + _editedDia, 'info');
...
function creEditFaseCancel(faseId) {
  var editForm = document.getElementById('cre-fase-editform-' + faseId);
