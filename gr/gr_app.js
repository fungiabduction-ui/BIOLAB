...
function actualizarSelectoresCT() {
    ...
    window.grToggleEdicionRegistro = function() {
        GR.modoEdicionRegistro = !GR.modoEdicionRegistro;
        const btn = document.getElementById('btnEditRegistroLotes');
        if (btn) {
            if (GR.modoEdicionRegistro) {
                btn.textContent = 'X Cerrar Edición';
    ...
    function renderizarRegistroLotes() {
        ...
        tbody.innerHTML = lotesOrdenados.map((lote, index) => {
            ...
            const btnDeleteHtml = GR.modoEdicionRegistro 
                ? `<button class="btn-delete-lote" onclick="event.stopPropagation(); window.eliminarLoteDesdeRegistro(${realIndex})" title="Eliminar lote">✕</button>` 
                : '';

            return `
                <tr style="cursor:pointer;" onclick="window.cargarLoteDesdeRegistro(${realIndex})" title="Clic para cargar">
                    <td>${lote.fecha || '-'}</td>
                    <td>${lote.id || '-'}</td>
                    <td>${lote.mezcla || '-'}</td>
...
// Sub-tab switcher (Formulación <-> Config)
GR.subTab = window.grSubTab = function grSubTab(t) {
    ...
    // Al entrar a config refrescar tablas
    if (t === 'cfg' && typeof renderizarBibliotecaEnConfig === 'function') {
        try { renderizarBibliotecaEnConfig(); } catch(e) {}
    }
};
...
// ==========================================
// SEGUIMIENTO DE LOTES (FASE POST-ESTERILIZACIÓN)
// ==========================================
GR.seguimientoNotas = [];

GR.onChangeSeguimientoEstado = window.grOnChangeSeguimientoEstado = function() {};

window.grRenderSeguimientoNotas = function() {
    const cont = document.getElementById('grSeguimientoNotas');
    if (!cont) return;
    if (GR.seguimientoNotas.length === 0) {
        cont.innerHTML = '';
        return;
    }
    
    cont.innerHTML = GR.seguimientoNotas.map(n => {
        let borderColor = 'var(--border)';
        let estadoStr = '⚪ Normal';
        let estadoColor = 'var(--tx2)';
        
        if(n.estado === 'green') { borderColor = '#70AD47'; estadoStr = '🟢 Positivo'; estadoColor = '#70AD47'; }
        else if(n.estado === 'yellow') { borderColor = '#FFC000'; estadoStr = '🟡 Atención'; estadoColor = '#FFC000'; }
        else if(n.estado === 'red') { borderColor = '#FF0000'; estadoStr = '🔴 Peligro'; estadoColor = '#FF0000'; }
        
        const numFrascos = n.frascos || n.contaminados || 0;
        let extraInfo = '';
        
        if(numFrascos > 0) {
            const numDias = n.dias !== undefined ? n.dias : 0;
            const strDias = numDias === 1 ? '1 día' : `${numDias} días`;
            extraInfo = `<div style="font-size:0.85rem; color:${estadoColor}; margin-top:4px; font-weight:600;">${estadoStr}: ${numFrascos} frasco(s) a los ${strDias} de iniciar el protocolo.</div>`;
        }

        return `<div class="gr-nota-entry" style="padding:10px 12px;margin-bottom:8px;background:var(--bg,#1D1D1D);border-left:3px solid ${borderColor};border-radius:6px;color:var(--tx,#F5F5F5)">
            <div class="nota-time" style="font-size:0.78rem;color:var(--tx2);font-weight:600;margin-bottom:4px">${n.ts}</div>
            <div class="nota-text" style="font-size:0.92rem;color:var(--tx,#F5F5F5)">${n.texto}</div>
            ${extraInfo}
        </div>`;
    }).join('');
...
window.grAddSeguimientoNota = function() {
    ...
    const frascosInput = document.getElementById('grSeguimientoFrascos');
    ...
    const frascos = parseInt(frascosInput?.value) || 0;
    
    // Calcular días pasados
    const loteFechaStr = document.getElementById('loteFecha')?.value;
    let dias = 0;
    if(loteFechaStr) {
        const [y, m, d] = loteFechaStr.split('-');
        const loteDate = new Date(y, m - 1, d);
        loteDate.setHours(0,0,0,0);
        const now = new Date();
        now.setHours(0,0,0,0);
        const diffTime = now - loteDate;
        dias = diffTime > 0 ? Math.floor(diffTime / (1000 * 60 * 60 * 24)) : 0;
    }
