...
window.grEliminarRegistro = grEliminarRegistro;
    ...
    function grGetNombreGeneticaPorId(fenId) {
        ...
        // cadena de nombres evita degradar a mostrar el id interno del nodo. SSoT del walk
        // crudo en shared/ge_resolve.js (mismo primitivo que usan CI y FR).
        if (window.GEResolve && typeof window.GEResolve.resolverNodoCrudo === 'function') {
            const resolved = window.GEResolve.resolverNodoCrudo(fenId);
            if (resolved) return resolved.chain.map(c => c.name).join(' / ');
        }
        return fenId;
    }
