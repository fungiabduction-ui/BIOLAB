...
2. Determine which file in `docs/lab-intelligence/hipotesis/` applies: `ge-ci-cilab.md` (GE/CI/CILAB), `gr.md`, `su.md`, `fr.md`, `cross-modulo.md` (spans more than one module), or `experimentos-en-cola.md` (conceptual experiment design). Read `docs/lab-intelligence/hipotesis/index.md` if unsure — create the whole `hipotesis/` directory (the 6 files above plus `index.md`, see "Hipótesis — formato" below) if it doesn't exist yet.
